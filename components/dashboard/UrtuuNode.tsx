...
export function UrtuuNode({ station, isSelected, onSelect, lockedLabel }: UrtuuNodeProps) {
  const { available, isCurrent, isDone, pos, name, icon = "location_on" } = station;
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-[9px] font-black text-primary uppercase tracking-widest">{name}</p>
      ...
      <button
        ...
        style={
          isCurrent
            ...
            : isDone
            ? { background: "var(--gold-main, #D4AF37)", borderColor: "var(--bg-abyss, #050608)" }
            : available
            ? {
                background: isSelected
                  ? "var(--gold-main,#D4AF37)"
                  : "color-mix(in oklch, var(--primary) 30%, var(--card))",
                borderColor: isSelected ? "white" : "var(--gold-dark,#AA8C2C)",
              }
      ...
      <div
        className={cn(
          ...
          available ? "glass border-primary/30" : "bg-slate-900/60 border border-slate-700/40"
        )}
