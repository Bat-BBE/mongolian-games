...
import {
  GameState, INITIAL_STATE, ShagaiSide,
  isDorvenBerkh,
} from "./fourBonusType"
...
function GameTable() {
  return (
    <>
      ...
      {/* Гол тавиур — зуйван хэлбэртэй felt (scale-ээр эллипс) */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow position={[0, 0.005, 0]} scale={[1, 0.64, 1]}>
        <circleGeometry args={[5.5, 64]} />
        <meshStandardMaterial color="#1a2e1a" roughness={0.93} />
      </mesh>

      {/* Алтан хүрээ */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.008, 0]} scale={[1, 0.64, 1]}>
        <ringGeometry args={[5.4, 5.65, 64]} />
        <meshStandardMaterial color="#c8a030" metalness={0.72} roughness={0.22} />
      </mesh>

      {/* Дотор алтан хүрээ */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.007, 0]} scale={[1, 0.64, 1]}>
        <ringGeometry args={[5.0, 5.1, 64]} />
        <meshStandardMaterial color="#a07820" metalness={0.5} roughness={0.4} />
      </mesh>

      {/* 4 булан дахь алтан тэмдэг */}
      ...
      {/* Үзэгдэхгүй хана */}
      {[
        { pos: [ 6, 1, 0] as [number,number,number], args: [0.2, 2, 12] as [number,number,number] },
        { pos: [-6, 1, 0] as [number,number,number], args: [0.2, 2, 12] as [number,number,number] },
        { pos: [0, 1,  5] as [number,number,number], args: [12, 2, 0.2] as [number,number,number] },
        { pos: [0, 1, -5] as [number,number,number], args: [12, 2, 0.2] as [number,number,number] },
      ].map((w, i) => (
...
function WinLightEffect({ active }: { active: boolean }) {
  ...
  return (
    <>
      <pointLight ref={light1} color="#f0c040" intensity={2} distance={6} position={[0, 3, 0]} />
      <pointLight ref={light2} color="#60c0ff" intensity={1.5} distance={6} position={[0, 2, 0]} />
    </>
  )
}

// ── Алтан частицын эффект ────────────────────
...
// Шидэлтийн хүч — тус бүр бага зэрэг өөр
...
interface SceneProps {
  state:         GameState
  throwParams:   { vel: [number,number,number]; angVel: [number,number,number] }[]
  isThrown:      boolean
...
function GameScene({ state, throwParams, isThrown, settledSides, onSettle, isWin }: SceneProps) {
...
export default function FourBonesGame() {
  ...
  const [throwParams,  setThrowParams]  = useState<ReturnType<typeof getThrowParams>[]>(
    [0,1,2,3].map(i => getThrowParams(i))
  )
  const [settledSides, setSettledSides] = useState<(ShagaiSide | null)[]>([null,null,null,null])
  ...
  // Нэг шагай тогтох — ref ашиглан найдвартай тооцно
  const handleSettle = useCallback((id: number, side: ShagaiSide) => {
    // Аль хэдийн тогтсон шагайг дахин тооцохгүй
    if (settledRef.current[id] !== null) return

    // Ref-д шууд бичнэ — async биш
    ...
    // Бүгд 4 тогтсон — ref-ээс уншдаг тул яг зөв
    if (settledCount.current >= 4 && !resultSentRef.current) {
      ...
      setTimeout(() => {
        ...
        setState(prev => ({
          ...prev,
          phase:   "result",
          history: [...prev.history, { sides, isDorvenBerkh: win, throwNumber: prev.totalThrows }],
          wins:       prev.wins + (win ? 1 : 0),
          streak:     win ? prev.streak + 1 : 0,
          bestStreak: win ? Math.max(prev.bestStreak, prev.streak + 1) : prev.bestStreak,
        }))
      }, 500)
    }
  }, [])
  ...
  return (
    <div style={{ width:"100%", height:"100%", position:"relative", background:"#080604", overflow:"hidden" }}>
      ...
      >
        ...
        <pointLight position={[ 4, 4,  4]} intensity={0.20} color="#c0d4ff" />
