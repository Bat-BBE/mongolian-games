...
export default function GameHistory({ history }: GameHistoryProps) {
  ...
  return (
    <div style={{
      ...
      {/* Сүүлийн 5 шидэлт */}
      <div style={{ display: "flex", gap: 6, marginBottom: isExpanded ? 14 : 0, flexWrap: "wrap" }}>
        ...
        {total > 5 && (
          <div style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "center", color: "#666", fontSize: 11 }}>
            +{total - 5}
          </div>
        )}
      </div>
          ...
          {SIDE_ORDER.map((side) => {
            ...
            return (
              <div key={side} style={{
                ...
                <div style={{ fontSize: 26, marginBottom: 4 }}>{info.symbol}</div>
                <div style={{ color: "#c8a030", fontSize: 12, letterSpacing: 1 }}>{info.name}</div>
                <div style={{ color: "white", fontSize: 22, fontWeight: "bold", margin: "4px 0" }}>{count}</div>
                <div style={{ color: "#666", fontSize: 11 }}>{pct.toFixed(1)}%</div>
                <div style={{ height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 2, marginTop: 6 }}>
                  <div style={{ height: "100%", width: `${pct}%`, background: "#c8a030", borderRadius: 2, transition: "width .5s" }} />
                </div>
              </div>
            );
          })}
