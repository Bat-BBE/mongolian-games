...
interface SingleShagaiProps {
  ...
  highlight:   boolean    // Дөрвөн бэрх үед гэрэлтэх
  resultSide:  ShagaiSide | null  // Settled болсны дараа
}

// Шагайн геометр — өмнөх загвартай ижил
function buildShagaiGeo(): THREE.BufferGeometry {
  ...
  for (let zi = 0; zi <= ZSEGS; zi++) {
    ...
    for (let ri = 0; ri <= RADIAL; ri++) {
      ...
      const yMod  = sin > 0 ? 1.0 - sin * 0.20 : 1.0 + Math.abs(sin) * 0.16
...
export default function SingleShagai({
  id, startPos, throwVel, throwAngVel,
  isThrown, onSettle, highlight, resultSide,
}: SingleShagaiProps) {
  ...
  const [ref, api] = useBox(() => ({
    mass:          0.55,
    position:      startPos,
    args:          [0.98, 0.70, 1.52] as [number,number,number],
    restitution:   0.20,
    friction:      0.82,
    linearDamping:  0.30,
    angularDamping: 0.25,
  ...
  useEffect(() => {
    const u1 = api.velocity.subscribe(v => { velRef.current = v })
    const u2 = api.position.subscribe(p => { posRef.current = p })
    return () => { u1(); u2() }
  }, [api])
  ...
  useEffect(() => {
    ...
    }, id * 60 + 40)  // Тус бүр бага зэрэг өөр хугацаанд
  ...
  const sideGlow  = resultSide ? SHAGAI_INFO[resultSide].glow  : "rgba(255,255,255,0.1)"
  ...
  return (
    ...
    >
      ...
      {/* Морийн тал тэмдэг */}
      <group position={[0, 0.30, 0.08]} rotation={[Math.PI/2, 0, 0]}>
        <mesh>
          <cylinderGeometry args={[0.045, 0.045, 0.65, 8]} />
          <meshStandardMaterial color="#b89060" roughness={0.45} transparent opacity={0.7} />
        </mesh>
      </group>

      {/* Тэмээний тал тэмдэг */}
      <group position={[0, -0.30, 0]} rotation={[0, 0, Math.PI]}>
        <mesh rotation={[Math.PI/2, 0, 0]}>
          <torusGeometry args={[0.11, 0.020, 8, 16, Math.PI]} />
          <meshStandardMaterial color="#907050" roughness={0.55} transparent opacity={0.6} />
        </mesh>
      </group>
